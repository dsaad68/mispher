...
extension TranscriptionViewModel {
    ...
    static func loadRecordingPresentation() -> RecordingPresentation {
        ...
        // unset default - to the Dynamic Island so dictation always shows a compact overlay.
        guard let stored, stored != .mainWindow else { return .dynamicIsland }
        return stored
    }

    static func loadAskPresentation() -> RecordingPresentation {
        let isIndependent = UserDefaults.standard.bool(forKey: "mispher.askPresentationIndependent")
        if isIndependent,
           let raw = UserDefaults.standard.string(forKey: askPresentationKey),
           let value = RecordingPresentation(rawValue: raw), value != .mainWindow {
            return value
        }
        // Not independent (or first launch): boot from the voice-modes value so Ask stays in sync.
