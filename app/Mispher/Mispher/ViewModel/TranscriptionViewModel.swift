...
final class TranscriptionViewModel {
    ...
    /// standalone hands-free mode). On by default.
    var silenceAutoEndEnabled = TranscriptionViewModel.loadBoolDefaultTrue(TranscriptionViewModel.silenceAutoEndEnabledKey) {
    ...
    var recordingPresentation: RecordingPresentation = TranscriptionViewModel.loadRecordingPresentation() {
        didSet {
            ...
            if !Self.loadBoolDefaultTrue("mispher.askPresentationIndependent") {
    ...
    /// time recording starts, instead of re-using its centred / last-dragged position. On by
    /// default. Persisted.
    var floatingFollowsPointer = TranscriptionViewModel.loadBoolDefaultTrue(TranscriptionViewModel.floatingFollowsPointerKey) {
    ...
    /// The MLX instruct model translation, rewrite, and cleanup default to (and migrate to).
    static let defaultTranslationModelId = "LiquidAI/LFM2.5-1.2B-Instruct-MLX-8bit"
