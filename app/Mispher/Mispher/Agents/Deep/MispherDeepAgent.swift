...
/// Mispher's concrete deep agent: a text **planner** (LFM2.5 8B-A1B) that breaks work into todos,
...
enum MispherDeepAgent {
    /// Build the deep agent. `textModel` is the planner (8B-A1B); `visionModel` is the VL model the
    /// `vision` subagent runs on and the only one that renders the forwarded image, or `nil` for a
    /// blind planner (vision "None": no vision subagent and no screen capture).
    ///
    ...
    static func make(
        ...
        // The `vision` subagent (and the screenshot capture that feeds it) only exists when there's a
        // VLM to run it — with the vision model set to "None" the planner runs blind and both drop.
        ...
        // Capability middleware - screen capture (non-attaching: the blind planner forwards captures
        // to the vision subagent), the clipboard, and Apple Notes - minus any the user turned off. MCP
        // tools (already loaded) ride along as one more middleware contributing to the main agent. Each
        // middleware brings its own prompt guidance with its tools. Screen capture is pointless without
        // a vision subagent to forward the (non-attached) capture to, so it rides along only with one.
        var middleware: [any AgentMiddleware] = []
        if vision != nil { middleware.append(ScreenshotMiddleware(attachToConversation: false)) }
        middleware += [ClipboardMiddleware(), AppleNotesMiddleware()]
        middleware = middleware.filter { !policy.disabledMiddleware.contains($0.name) }
        ...
        let expansion = policy.expand(extraDefaults: mcpDefaults)

        // Gating (and real-disk access) only applies when there's an approver to ask; otherwise
        // the agent runs ungated on the in-memory scratch filesystem, as before. Deny-mode tools
        // are auto-rejected before they ever reach the user.
        ...
        return createDeepAgent(
            ...
            memory: memory,
            backend: gated
                ? LocalFilesystemBackend(rootURL: workingDirectory ?? FileManager.default.homeDirectoryForCurrentUser)
                : nil,
